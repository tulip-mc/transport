...
## player inside?
execute as @e[tag=gateway.generic,tag=!gateway.to_transport_player] at @s if entity @a[distance=..2,limit=1,tag=!gateway.player_to_transport,tag=!gateway.player_on_cooldown] run tag @s add gateway.to_transport_player
execute as @e[tag=gateway.generic,tag=!gateway.to_transport_player] at @s as @a[distance=..2,limit=1,tag=!gateway.player_to_transport,tag=!gateway.player_on_cooldown] run tag @s add gateway.player_to_transport
